...
def halt_loss(
    halt_score: torch.Tensor, y_prob: torch.Tensor, y_true: torch.Tensor
) -> torch.Tensor:
    with torch.no_grad():
...
def main() -> None:
    ...
    scheduler = torch.optim.lr_scheduler.ConstantLR(optimizer, factor=1.0, total_iters=cfg.epochs)
