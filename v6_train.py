...
class Config:
    ...
    # V6: dz 128→256, dh 256→512 to rebalance encoder:reasoner ratio.
    # With convnext_tiny (27.8M) this brings reasoner to ~11M, ratio ~2.5:1
    # vs v5's 23:1.  Still within the 50M budget.
    latent_dim_h: int = 512
    latent_dim_z: int = 256

    # --- recursive loop ---
    max_steps: int = 6
    min_steps: int = 3   # was 2; give loop more steps to train on
    K_inner:   int = 3

    # --- adaptive halting ---
    halt_improvement_threshold: float = 0.005
    # V6: longer warmup so the closed-loop dynamics stabilise before halting gates inference.
    halt_warmup_epochs: int = 8
