...
class Config:
    # --- identity ---
    dataset_name: str = "lung"              # local dataset folder under data/
    ...
    # --- adaptive halting ---
    # The halt head is now supervised on per-step Dice *improvement* rather
    # than absolute Dice.  At inference a sample is considered "done" when its
    # predicted improvement drops below halt_improvement_threshold.
    # This works equally well for weak (scratch) and strong (pretrained) models
    # because the signal is relative, not absolute.
    halt_improvement_threshold: float = 0.005   # stop when predicted gain < 0.5%
    # Warm-up: keep halting disabled for the first N epochs so the halt head
    # gets enough training signal before it is used for inference gating.
    halt_warmup_epochs: int = 5
    ...
    # --- budget ---
    max_params_millions: float = 250.0
...
def halt_loss(
    halt_score:  torch.Tensor,
    y_prob:      torch.Tensor,
    y_true:      torch.Tensor,
    prev_dice:   Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """
    Supervise halt_score [B] to regress the per-step Dice *improvement*.

    Target = Dice(current) - Dice(previous step).
    For step 0, prev_dice is None so improvement = Dice(current) — this
    gives the halt head a non-trivial positive signal on the first step
    without needing a special case.

    Improvement is clamped to [0, 1] before use as the regression target.
    A halt score near 0 means "still improving, keep going."
    A halt score near 1 means "marginal gain, safe to stop."

    Why improvement rather than absolute Dice:
      - A pretrained model produces Dice > 0.82 from epoch 1,
        causing absolute-Dice supervision to saturate the halt head
        immediately.
      - Improvement is invariant to the encoder's capability level — a model
        that goes from 0.88 to 0.882 produces the same halt signal as one
        that goes from 0.40 to 0.402.
    """
    with torch.no_grad():
        cur_dice = dice_score((y_prob.float() > 0.5).float(), y_true.float())  # [B]
        if prev_dice is None:
            improvement = cur_dice.clamp(0.0, 1.0)
        else:
            improvement = (cur_dice - prev_dice).clamp(0.0, 1.0)
    # Invert: high halt_score = small improvement (ready to stop)
    halt_target = 1.0 - improvement
    return F.mse_loss(halt_score.float(), halt_target.detach())
...
def total_loss(
    ...
    prev_dice: Optional[torch.Tensor] = None   # tracks Dice from previous step
    ...
    for w, out in zip(weights, outputs):
        ...
        l_halt = halt_loss(out["halt_score"], out["y_prob"], y_true, prev_dice)

        # Update prev_dice for next step (detached — no gradient through it)
        with torch.no_grad():
            prev_dice = dice_score(
                (out["y_prob"].float() > 0.5).float(), y_true.float()
            ).detach()
...
def run_train_epoch(
    ...
    meter   = {k: 0.0 for k in ["loss", "seg", "bnd", "unc", "kl", "halt",
    ...
    for bidx, batch in enumerate(loader, start=1):
        ...
        if bidx % cfg.log_step_interval == 0 or bidx == total_b:
            ...
            msg = (
                ...
                f"loss={wl['loss']:.4f}  dice={wl['dice']:.4f}  iou={wl['iou']:.4f}  "
                f"[seg={wl['seg']:.4f}  bnd={wl['bnd']:.4f}  unc={wl['unc']:.4f}  "
                f"kl={wl['kl']:.6f}  halt={wl['halt']:.4f}]"
            )
