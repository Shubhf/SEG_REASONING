...
class Config:
    ...
    # --- encoder ---
    # Encoder options and approximate backbone parameter counts:
    #   resnet18        11.7 M   lightweight, fast
    #   resnet34        21.8 M   deeper basic-block ResNet
    #   resnet50        25.6 M   bottleneck ResNet, strong ImageNet features
    #   resnet101       44.5 M   deep bottleneck ResNet
    #   resnet152       60.2 M   deepest ResNet
    #   convnext_tiny   28.6 M   efficient modern architecture
    #   convnext_small  50.2 M   mid-size ConvNeXt
    #   convnext_base   88.6 M   strong accuracy/size tradeoff
    #   convnext_large 197.8 M   highest capacity ConvNeXt
    ...
    # --- training ---
    epochs:       int   = 200
    # Decoder / head learning rate (randomly initialised modules)
    lr_decoder:   float = 1e-3
    # Encoder learning rate — should be much lower than decoder when pretrained,
    # so pretrained features are fine-tuned rather than destroyed.
    # Rule of thumb: lr_encoder = lr_decoder / 10 for ResNet,
    #                             lr_decoder / 5  for ConvNeXt.
    ...
    # --- adaptive halting ---
    # V7: uncertainty-based threshold — halt when mean uncertainty < (1 - threshold)
    halt_improvement_threshold: float = 0.1
    halt_warmup_epochs: int = 0
...
def resolve_run_dir(cfg: Config) -> str:
    """
    Output path: RESULTS/<dataset_name>_<encoder>/run_<NNN>
...
def setup_logger(run_dir: str) -> logging.Logger:
    ...
    # Clear any existing handlers to be notebook/re-run safe
...
class LocalNPYDataset(Dataset):
    ...
    Images are returned normalised to [0, 1].  The encoder applies
    ImageNet mean/std normalisation internally, so no further preprocessing
    is needed here.
    """
    ...
    def _mask_to_np(self, mask) -> np.ndarray:
        ...
        if arr.ndim == 3 and arr.shape[0] == 1:
            arr = arr[0]              # (H, W) from channel-first
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # Normalise to [0, 1] — encoder applies ImageNet stats internally
...
def uncertainty_loss(
    u: torch.Tensor, y_prob: torch.Tensor, y_true: torch.Tensor
) -> torch.Tensor:
    p = y_prob.float().detach()
    error = (p - y_true.float()).abs()
    # Prediction entropy: high at boundary pixels (p≈0.5) regardless of correctness.
    # Using max(error, entropy) prevents uncertainty from collapsing to zero once
    # the model becomes accurate — boundary/ambiguous regions always stay uncertain.
    entropy = -(p * (p + 1e-8).log() + (1 - p) * (1 - p + 1e-8).log()) / _LN2
    unc_target = torch.max(error, entropy).clamp(0.0, 1.0)
    return F.l1_loss(u.float(), unc_target)


def kl_loss(mu: torch.Tensor, logvar: torch.Tensor) -> torch.Tensor:
    return (
        -0.5 * (1 + logvar.float() - mu.float().pow(2) - logvar.float().exp())
    ).mean()


def halt_loss(outputs: List[Dict]) -> torch.Tensor:
    """
    Supervise halt_score on prediction-change signal across adjacent steps.

    halt_target = 1 - clip(delta_prob * scale, 0, 1)
      high (near 1) when mask stopped changing  → halt
      low  (near 0) when mask still changing    → keep going

    This makes halting genuinely adaptive: easy cases stabilize quickly,
    hard cases keep changing and earn more passes.
    """
    if len(outputs) < 2:
        return torch.tensor(0.0, device=outputs[0]["halt_score"].device)
    loss = torch.tensor(0.0, device=outputs[0]["halt_score"].device)
    for i in range(1, len(outputs)):
        y_curr = outputs[i]["y_prob"].float()
        y_prev = outputs[i - 1]["y_prob"].float().detach()
        delta_prob  = (y_curr - y_prev).abs().mean(dim=(1, 2, 3))
        halt_target = (1.0 - (delta_prob * 20.0).clamp(0.0, 1.0)).detach()
        loss = loss + F.mse_loss(outputs[i]["halt_score"].float(), halt_target)
    return loss / (len(outputs) - 1)


def monotonicity_loss(outputs: List[Dict], y_true: torch.Tensor) -> torch.Tensor:
    """Penalize any step where soft-Dice drops vs the previous step."""
    loss = torch.tensor(0.0, device=y_true.device)
    for i in range(1, len(outputs)):
        dice_curr = dice_score(outputs[i]["y_prob"].float(),   y_true.float())
        dice_prev = dice_score(outputs[i-1]["y_prob"].float(), y_true.float()).detach()
        loss = loss + F.relu(dice_prev - dice_curr).mean()
    return loss / max(len(outputs) - 1, 1)
...
def total_loss(
    ...
    # Halt loss: one value across the full step sequence (not per-step weighted)
    l_halt = halt_loss(outputs)
    total  = total + cfg.halt_weight * l_halt
    parts["halt"] = l_halt.item()

    # Monotonicity penalty: computed once across all steps (not per-step weighted)
    l_mono = monotonicity_loss(outputs, y_true)
    total  = total + cfg.monotonicity_weight * l_mono
    parts["mono"] = l_mono.item()
...
def build_optimizer(model: HiReMed, cfg: Config) -> torch.optim.AdamW:
    """
    Two parameter groups with different learning rates:
      - encoder:  lr_encoder  (smaller — fine-tuning pretrained weights)
      - decoder:  lr_decoder  (larger  — training randomly-initialised weights)

    Using a single learning rate for everything causes the pretrained encoder
    to be overwritten in the first few epochs.
    """
    return torch.optim.Adam(
        ...
        # AdamW requires at least a default lr; it is overridden per group above
        lr=cfg.lr_decoder,
        weight_decay=cfg.weight_decay,
    )
...
def eval_single_pass(model: HiReMed, loader: DataLoader, cfg: Config) -> Dict:
    """Exactly 1 refinement step per sample (force_steps=1)."""
    model.eval()
    meter = _empty_meter()
    meter["mean_unc"] = 0.0
    n = 0
    for batch in loader:
        ...
        for k in METRIC_KEYS:
            meter[k] += mets[k]
        meter["mean_unc"] += outputs[0]["uncertainty"].mean().item()
        n += 1
    for k in meter:
        meter[k] /= max(n, 1)
    return meter
...
def eval_adaptive(
    ...
    loader:        DataLoader,
    cfg:           Config,
    current_epoch: int = 9999,
) -> Dict:
    """
    Adaptive halting — min_steps to max_steps, exits early per-sample.

    During the warm-up period (current_epoch <= halt_warmup_epochs) halting
    is disabled and all max_steps are run, so the halt head has time to learn
    before it controls inference.
    """
    model.eval()
    meter = _empty_meter()
    meter["avg_steps"] = 0.0
    meter["mean_unc"]  = 0.0
    n = 0
    halting_active = current_epoch > cfg.halt_warmup_epochs
    for batch in loader:
        images = batch["image"].to(cfg.device, non_blocking=True)
        masks  = batch["mask"].to(cfg.device,  non_blocking=True)
        # During warm-up run full max_steps so metrics are still meaningful
        force  = None if halting_active else cfg.max_steps
        ...
        meter["mean_unc"]  += outputs[-1]["uncertainty"].mean().item()
        n += 1
    for k in meter:
        meter[k] /= max(n, 1)
    return meter
...
def eval_both_modes(
    ...
    """Runs both modes, prints a side-by-side table, returns combined dict."""
    sp  = eval_single_pass(model=model, loader=loader, cfg=cfg)
    adp = eval_adaptive(model=model, loader=loader, cfg=cfg, current_epoch=current_epoch)
    ...
    for k in METRIC_KEYS:
        ...
        lines.append(
            f"  {k:<14}  {sp[k]:>10.4f}  {adp[k]:>10.4f}  {sign}{delta:>7.4f}"
        )
    lines.append(
        f"  {'avg_steps':<14}  {'1':>10}  {adp['avg_steps']:>10.2f}  {'':>8}"
    )
    lines.append(
        f"  {'mean_unc':<14}  {sp['mean_unc']:>10.4f}  {adp['mean_unc']:>10.4f}  {'':>8}"
    )
    lines.append(f"{'─'*62}")

    block = "\n".join(lines)
    (logger.info if logger else print)(block)
    return combined
...
def run_train_epoch(
    ...
    """Full training pass with per-window step logging."""
    meter   = {k: 0.0 for k in ["loss", "seg", "bnd", "unc", "kl", "halt", "mono",
                                  "mean_unc", "dice", "iou", "precision", "recall", "accuracy"]}
    ...
    for bidx, batch in enumerate(loader, start=1):
        ...
        mets         = compute_metrics(outputs[-1]["y_logit"], masks, cfg.threshold)
        mean_unc     = outputs[-1]["uncertainty"].mean().item()
        ...
        if bidx % cfg.log_step_interval == 0 or bidx == total_b:
            wl = {k: window[k] / max(wn, 1) for k in window}
            msg = (
                f"Epoch {epoch:03d}  step {bidx:04d}/{total_b:04d}  "
                f"loss={wl['loss']:.4f}  dice={wl['dice']:.4f}  iou={wl['iou']:.4f}  mean_unc={wl['mean_unc']:.4f}  "
                f"[seg={wl['seg']:.4f}  bnd={wl['bnd']:.4f}  unc={wl['unc']:.4f}  "
                f"kl={wl['kl']:.6f}  halt={wl['halt']:.4f}  mono={wl['mono']:.4f}]"
            )
...
def print_model_summary(
    ...
    if HAS_TORCHINFO:
        ...
        lines.append("torchinfo not installed — per-module param table:")
        lines.append(f"  {'Module':<55} {'Params':>12}")
        lines.append("  " + "─" * 68)
        for name, mod in model.named_modules():
            own = sum(p.numel() for p in mod.parameters(recurse=False) if p.requires_grad)
            if own > 0:
                lines.append(f"  {name:<55} {own:>12,}")

    lines += [
        "",
        f"Encoder          : {model.encoder_name}  (pretrained={model.encoder_pretrained})",
        f"Encoder channels : s1={model.s1_ch}, s2={model.s2_ch}, "
        f"s3={model.s3_ch}, s4={model.s4_ch}",
        "",
        "Per-submodule parameter breakdown:",
        f"  {'Submodule':<30} {'Params (M)':>12}  {'% trainable':>12}",
        "  " + "─" * 58,
    ]
    ...
    for attr in ["encoder", "z_proj", "low", "high", "decoder", "unc_head", "halt_head"]:
        ...
        if sub is None:
            continue
        p = sum(x.numel() for x in sub.parameters() if x.requires_grad)
        lines.append(f"  {attr:<30} {p/1e6:>12.3f}  {100*p/total_tr:>11.1f}%")
    lines.append("  " + "─" * 58)
    lines.append(f"  {'TOTAL trainable':<30} {total_tr/1e6:>12.3f}  {'100.0':>11}%")
    frozen = sum(p.numel() for p in model.parameters() if not p.requires_grad)
    if frozen > 0:
        lines.append(f"  {'TOTAL frozen':<30} {frozen/1e6:>12.3f}")
    lines.append("")
    ...
    if save_path:
        with open(save_path, "w") as f:
            f.write(output)
        (logger.info if logger else print)(f"  Model summary saved → {save_path}")
...
def main() -> None:
    ...
    # --- model ---
    logger.info(
        f"Building model — encoder={cfg.encoder_name}  "
    ...
    enc_params_m = sum(p.numel() for p in model.encoder.parameters() if p.requires_grad) / 1e6
    dec_params_m = sum(p.numel() for p in model.decoder_parameters()) / 1e6

    logger.info(f"  Encoder backbone     : {ENCODER_PARAMS_M.get(cfg.encoder_name, '?'):.1f}M (reference)")
    logger.info(f"  Trainable — encoder  : {enc_params_m:.2f}M  (lr={cfg.lr_encoder:.1e})")
    logger.info(f"  Trainable — decoder  : {dec_params_m:.2f}M  (lr={cfg.lr_decoder:.1e})")
    logger.info(f"  Trainable — total    : {params_m:.2f}M")
    logger.info(f"  All params (total)   : {all_params_m:.2f}M")
    logger.info(
        f"  Encoder channels     : s1={model.s1_ch}, s2={model.s2_ch}, "
        f"s3={model.s3_ch}, s4={model.s4_ch}"
    )
    ...
    print_model_summary(
        model, cfg,
        save_path=os.path.join(run_dir, "model_summary.txt"),
        logger=logger,
    )

    if params_m > cfg.max_params_millions:
        raise ValueError(
            f"Model has {params_m:.2f}M trainable params, "
            f"exceeds budget {cfg.max_params_millions:.2f}M"
        )
    ...
    logger.info(
        ...
        f"lr_encoder={cfg.lr_encoder:.1e}  lr_decoder={cfg.lr_decoder:.1e}  "
        f"log_interval={cfg.log_step_interval} steps"
    )

    for epoch in range(1, cfg.epochs + 1):
        t0 = time.time()

        # Log the LR that will be USED for this epoch's training (before scheduler.step)
        ...
        train_m  = run_train_epoch(model, train_loader, optimizer, cfg, epoch, logger)

        # Step the scheduler AFTER the training pass, BEFORE logging the next epoch's LR
        scheduler.step()

        in_warmup = epoch <= cfg.halt_warmup_epochs
        val_both = eval_both_modes(
            model, val_loader, cfg,
            label=f"val epoch {epoch}" + (" [halt warm-up]" if in_warmup else ""),
            logger=logger,
            current_epoch=epoch,
        )
        dt = time.time() - t0
        ...
        row = {
            "epoch":   epoch,
            # LR used during this epoch's training
            "lr_encoder": enc_lr,
            "lr_decoder": dec_lr,
            "time_s":  round(dt, 1),
            "train":   train_m,
        ...
        if val_dice_adaptive > best_val_dice:
            ...
            logger.info(
                f"  ↑ New best  val_dice(adaptive)={best_val_dice:.4f}  checkpoint saved"
            )
    ...
    # -----------------------------------------------------------------------
    # Visuals — both modes for val and test
    # -----------------------------------------------------------------------
    for split_name, loader in [("val", val_loader), ("test", test_loader)]:
        for mode in ("adaptive", "single_pass"):
            save_predictions(
                model, loader, cfg,
                out_dir=os.path.join(vis_dir, split_name, mode),
