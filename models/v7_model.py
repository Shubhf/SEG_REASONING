"""
HiReMed V7 — Uncertainty-guided closed-loop refinement with dropout regularisation.
...
class LowLevelRefinement(nn.Module):
    ...
    *what was predicted* so far and can compute a correction, rather than
    ...
    """
    def __init__(
        self, dz: int, dh: int, s2_ch: int,
        y_prob_ch: int = 1, u_prev_ch: int = 1, dropout_p: float = 0.1,
    ):
        super().__init__()
        self.conv1 = ConvGNAct(dz + dh + s2_ch + y_prob_ch + u_prev_ch, dz, 3, 1, 1)
        self.conv2 = nn.Sequential(
    ...
    def forward(
        ...
        up_res = F.interpolate(u_prev,      size=(hz, wz), mode="bilinear", align_corners=False)
        cat    = torch.cat([z, h_map, s2_res, yp_res, up_res], dim=1)
        return self.act(self.drop(self.conv2(self.conv1(cat))) + z)
...
class HighLevelTransition(nn.Module):
    ...
    def forward(
        ...
        u      = self.drop(F.gelu(self.fc1(torch.cat([h, z_pool, s3_pool], dim=1))))
        u      = self.fc2(u)
        mu     = self.mu_head(u)
...
class HiReMed(nn.Module):
    ...
    def __init__(
        ...
        self.low  = LowLevelRefinement(dz, dh, s2_ch, dropout_p=dropout_p)
        self.high = HighLevelTransition(dh, dz, s3_ch, dropout_p=dropout_p)
    ...
    def forward(
        ...
        for step in range(max_s):
            ...
            # 1. Inner spatial refinement  (K_inner iterations, uncertainty-guided closed-loop)
            # Recursive input dropout: randomly zero out feedback for some samples during
            # training so the model must learn to use it, not ignore it.
            if self.training and self.recursive_dropout_p > 0.0 and step > 0:
                keep = (torch.rand(B, 1, 1, 1, device=dev) > self.recursive_dropout_p).to(dtype)
                y_prob_in = y_prob_prev * keep
                u_prev_in = u_prev * keep
            else:
                y_prob_in = y_prob_prev
                u_prev_in = u_prev

            for _ in range(self.K_inner):
                z = self.low(z, h, s2, y_prob_in, u_prev_in)
            ...
            # Feed current prediction and uncertainty back into next step
            ...
            # halt_score ≈ 1 - delta_prob: high = prediction stabilized = stop, low = still changing = keep going.
            # Deactivate a sample when halt_score >= (1 - halt_improvement_threshold).
