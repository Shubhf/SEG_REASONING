...
class LowLevelRefinement(nn.Module):
    ...
    s2 is downsampled to match z's spatial size before concatenation.
    Output: z  [B, dz, Hz, Wz]
    """
    ...
    def forward(self, z: torch.Tensor, h: torch.Tensor, s2: torch.Tensor) -> torch.Tensor:
        ...
        # Broadcast global h to spatial grid
        h_map  = h.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, hz, wz)
        # Resize s2 to match z — z is at H/32, s2 is at H/8, so this downsamples
...
class HiReMed(nn.Module):
    ...
    Constructor args:
        ...
        halt_threshold      inference-time per-sample exit threshold
    ...
    def forward(
        ...
        for step in range(max_s):
            ...
            # 1. Inner spatial refinement  (K_inner iterations)
